...
def _build_report(date_str: str, events: list[dict[str, Any]]) -> str:
    ...
    # 6. Security review                                                   #
    # ------------------------------------------------------------------ #
    _build_security_section(lines, events)
...
def _build_security_section(lines: list[str], events: list[dict[str, Any]]) -> None:
    """Append a Security Review section to the report lines list."""
    tool_events = [e for e in events if e.get("type") == "tool_call"]

    # --- Injection signals ------------------------------------------------
    injection_events = [
        e for e in events
        if e.get("type") == "channel_event" and e.get("injection_signal")
    ]

    # --- docker_exec with security metadata --------------------------------
    docker_events = [
        e for e in tool_events
        if e.get("tool") == "docker_exec" and e.get("security")
    ]

    # --- Non-zero / failed exits (exec + docker_exec) ----------------------
    exit_failures = [
        e for e in tool_events
        if e.get("tool") in ("docker_exec", "exec")
        and e.get("security", {}).get("exit_code", 0) not in (0, None)
        and not e.get("security", {}).get("timed_out")
    ]
    timed_out_events = [
        e for e in tool_events
        if e.get("security", {}).get("timed_out")
    ]

    # Only emit the section if there's something interesting.
    if not (injection_events or docker_events or exit_failures or timed_out_events):
        return
    ...
    # Injection signals
    if injection_events:
        lines.append(f"### ⚠ Injection Signals ({len(injection_events)})")
        lines.append("")
        lines.append("Inbound messages matched a known prompt-injection pattern.")
        lines.append("")
        for ev in injection_events[:10]:
            ts = ev.get("ts", "?")[:19]
            ch = ev.get("channel", "?")
            sid = ev.get("session_id", "?")
            lines.append(f"- `{ts}` channel=**{ch}** session={sid}")
        lines.append("")
    else:
        lines.append("### Injection Signals: none detected")
        lines.append("")

    # docker_exec hash table
    if docker_events:
        lines.append(f"### docker_exec Executions ({len(docker_events)})")
        lines.append("")
        lines.append("| Time | OK | Lines | SHA256 (16) | Exit | Stderr | First line |")
        lines.append("|---|---|---|---|---|---|---|")
        for ev in docker_events[:30]:
            ts = ev.get("ts", "?")[11:19]  # HH:MM:SS
            ok = "✓" if ev.get("ok") else "✗"
            sec = ev.get("security", {})
            lines_n = sec.get("code_lines", "?")
            sha = sec.get("code_sha256", "?")
            exit_code = sec.get("exit_code", 0)
            had_stderr = "yes" if sec.get("had_stderr") else "—"
            first = sec.get("code_first_line", "")[:40]
            lines.append(f"| {ts} | {ok} | {lines_n} | `{sha}` | {exit_code} | {had_stderr} | `{first}` |")
        if len(docker_events) > 30:
            lines.append(f"| ... | | | | | | _+{len(docker_events)-30} more_ |")
        lines.append("")

    # Exit failures
    if exit_failures or timed_out_events:
        lines.append(f"### Abnormal Exits ({len(exit_failures) + len(timed_out_events)})")
        lines.append("")
        for ev in (exit_failures + timed_out_events)[:10]:
            ts = ev.get("ts", "?")[:19]
            tool = ev.get("tool", "?")
            sec = ev.get("security", {})
            note = "TIMEOUT" if sec.get("timed_out") else f"exit {sec.get('exit_code', '?')}"
            stderr = sec.get("stderr_tail", "")[:80]
            lines.append(f"- `{ts}` **{tool}** — {note}" + (f": `{stderr}`" if stderr else ""))
        lines.append("")
