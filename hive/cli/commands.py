...
app = typer.Typer(
    name="hive",
    help=f"{__logo__} hive - Queen Agent",
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".hive" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} hive[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} hive v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize hive configuration and workspace."""
    from hive.config.loader import get_config_path, load_config, save_config
    from hive.config.schema import Config
    from hive.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} hive is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.hive/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]hive agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram? Configure in ~/.hive/config.json[/dim]")
...
def gateway(
    ...
    from hive.config.loader import load_config, get_data_dir
    from hive.bus.queue import MessageBus
    from hive.agent.loop import AgentLoop
    from hive.channels.manager import ChannelManager
    ...
    console.print(f"{__logo__} Starting hive gateway on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show hive runtime logs during chat"),
):
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]hive-queen is thinking...[/dim]", spinner="dots")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".hive" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # hive/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall hive-office-agents")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from hive.config.loader import get_data_dir
    from hive.cron.service import CronService
...
def cron_add(
    ...
    from hive.config.loader import get_data_dir
    from hive.cron.service import CronService
    from hive.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} hive Status\n")
