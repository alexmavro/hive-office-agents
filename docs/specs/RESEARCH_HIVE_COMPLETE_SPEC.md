...
- [Appendix C: VPS-Claude Reference Guide](#appendix-c-vps-claude-reference-guide)
...
**Companion file:** `RESEARCH_HIVE_VPS_REFERENCE.md` contains skills to reference and Gemini API configuration. Read after understanding this spec.
...
**Model assignment:**
- Librarian: Gemini 2.0 Flash (retrieval doesn't need smart model)
- Architect: Gemini 2.0 Flash Thinking (structure benefits from reasoning)
- Writer: Gemini 2.5 Pro (needs quality to sound like Alex)
- Critic: Gemini 2.0 Flash (rule-checking is mechanical)

**Cost estimate during development:** ~$0.10-0.30/day (within free tier limits)

**IMPLEMENTATION NOTE:** See `RESEARCH_HIVE_VPS_REFERENCE.md` for Gemini API setup and configuration patterns. Alex has Gemini API credits.
...
- Config files created (including models.yaml for Gemini API)
- Gemini API key configured in .env
- Gemini API connection tested
- Git initialized

**Success criteria:**
- `python -c "import lancedb, sentence_transformers, gradio, google.generativeai"` works
- Syncthing test file appears within 60 seconds
- Config files validate (correct YAML syntax)
- Gemini API test returns response

**Test:** 
- Drop test file on desktop → appears in ~/content-vault/ on VPS
- Run Gemini test script → returns "Hello" response

**IMPORTANT:** See `RESEARCH_HIVE_VPS_REFERENCE.md` for Gemini API setup steps and testing script.
...
# LLM providers
google-generativeai>=0.8.0  # Gemini API (primary for workers)
# anthropic>=0.8.0  # Optional: if using Claude for workers instead
# openai>=1.0.0  # Optional: if using OpenAI for workers instead
...
## Appendix C: VPS-Claude Reference Guide

**This spec is complete on requirements and architecture.**

**For implementation helpers, see:** `RESEARCH_HIVE_VPS_REFERENCE.md`

That companion file contains:
- Skills from awesome-agent-skills repo to reference (not copy)
- Gemini API configuration for workers (Alex has credits)
- Model selection guidance (Flash vs Pro)
- Code integration patterns
- Testing approaches

Read it AFTER you understand this spec. It shows WHERE to look and HOW to configure, not WHAT to build.
