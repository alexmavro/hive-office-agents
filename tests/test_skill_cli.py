...
def test_skill_init_scaffolds_correctly(tmp_path: Path):
    ...
    # Run init command
    result = runner.invoke(skill, ["init", "my-test-skill", "--desc", "Test skill", "--workspace", str(tmp_path)])
...
def test_skill_init_rejects_invalid_name(tmp_path: Path):
    ...
    result = runner.invoke(skill, ["init", "My Invalid Skill!", "--workspace", str(tmp_path)])
...
def test_skill_package_zips_correctly(tmp_path: Path):
    ...
    # 1. Init a skill
    runner.invoke(skill, ["init", "hello-world", "--workspace", str(tmp_path)])
    ...
    result = runner.invoke(skill, ["package", "hello-world", "--workspace", str(tmp_path), "--outdir", str(out_dir)])
...
def test_skill_package_fails_on_missing_frontmatter(tmp_path: Path):
    ...
    # 3. Package it
    result = runner.invoke(skill, ["package", "bad-skill", "--workspace", str(tmp_path), "--outdir", str(tmp_path)])
